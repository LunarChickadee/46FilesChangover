...
mailaddress, mailcopies, mailheader, messageBody, newyear, newpfile, rollingdisc,ID, taxable, treeshipstate
...
if OrderNo>400000 and OrderNo<500000
    taxstates="AK,CT,GA,IL,IN,KY,MI,MN,NC,NJ,NY,OH,PA,RI,WA,WI,WV"
    tshipexemptions="VT"
    noship="CO,MD,ME,UT,VA,WY"
    noshipexemptions="MA"
    ...
    case arraycontains(taxstates,TaxState,",")=-1 or Notes4 contains "AKShippingIsTaxable"
        TaxedAmount=taxable+Surcharge+«$Shipping»
    
    case arraycontains(tshipexemptions,TaxState,",")=-1
        taxable=(TaxTotal-?(VolDisc≠0,round(float(TaxTotal)*float(Discount)+.0001,.01),0)
        -?(MemDisc≠0,round((TaxTotal*0.01)+.0001,.01),0)+round(«$Shipping»*float(divzero(TaxTotal,Subtotal))+.0001,.01))
        TaxedAmount=taxable
...
if OrderNo > 700000 and OrderNo < 1000000
    case TaxState contains "VT" or TaxState contains "RI" or TaxState contains "CT"
        ...
        arrayfilter Order, taxitems,¬,?(
            (val(extract(extract(Order,¶,seq()),¬,2))<=797) 
            or (val(extract(extract(Order,¶,seq()),¬,2))>=800 and val(extract(extract(Order,¶,seq()),¬,2))<=4699)
...
if OrderNo > 300000 and OrderNo < 400000
    ...
    case arraycontains(noship,TaxState,",")=-1 or (TaxState contains "AK" and Notes4 notcontains "AKShippingIsTaxable")
        TaxedAmount=taxable
    case arraycontains(taxstates+","+noship,TaxState,",")=-1 or Notes4 contains "AKShippingIsTaxable"
...
local p1,p2,p3,p4,p5,p6,p7,p8,p14,p50
p1="AL,AR,AZ,CA,FL,GA,HI,LA,MS,NC,SC,TN,TX"
p2="DC,DE,KY,MD,MO,OK,VA"
p3="IN,NJ,NM,WV"
p4="IL,OH"
p5="IA,KS,NE,NV,OR,WA"
p6="PA"
p7="CT"
p8="RI"
p14="CO,ID,ND,MT,SD,UT,WY"
p50="AK"
...
if «Notes2» contains "group order" or «1SpareText» contains "G" or «GroupMarker» contains "G"
...
//South/Central WI
    Case (Z>53000 and Z<54500)
    or (Z>54600 and Z<54800)
    or (Z>54900 and Z<55000)
        Pool=13
        
//South/Central MN
    Case (Z>55000 and Z<55600)
    or (Z>55900 and Z<56600)
        Pool=13
...
//Northern WI
    Case (Z>54500 and Z<54600)
    or (Z>54800 and Z<54900)
        Pool=27      
        
//Northern MN
    Case (Z>55600 and Z<55900)
    or (Z>56600 and Z<56800)
        Pool=27
...
case OrderNo > 600000 and OrderNo < 700000
    ...
    openfile "OrderRetotaller"
    openfile "&@checkorder"
    call "mtbulkretotaller"
defaultcase
    message "No bulk prices in this branch"
    stop
endcase
call ".retotal"
...
global checkorder, taxstate, shipcode
...
Case ShipCode="X" or Sta="AK"
«$Shipping»=?(AdjTotal<129,27, round(float(AdjTotal)*float(.21)+.001,.01))

;if rayj="N"
;«$Shipping»=?(AdjTotal<125,25, round(float(AdjTotal)*float(.2)+.001,.01))
;endif
;if rayj="Y"
;«$Shipping»=?(AdjTotal≤59, 10, round(float(AdjTotal)*float(.17)+.001,.01))
;endif


Case ShipCode="U"
if rayj="N"
«$Shipping»=?(AdjTotal≤147,25, round(float(AdjTotal)*float(.17)+.001,.01))
endif
if rayj="Y"
«$Shipping»=?(AdjTotal≤59, 10, round(float(AdjTotal)*float(.17)+.001,.01))
endif
...
Case ShipCode="T" or ShipCode="P"
«$Shipping»=0
DefaultCase «$Shipping»=0
EndCase
...
case OrderNo > 700000 and OrderNo < 1000000
    ;; SEEDS
    Discount = ?(ShipCode="F", 0, Discount) ;; fundraisers don't get discounts
    VolDisc=float(Subtotal)*float(Discount)
    MemDisc=?(MemDisc>0, float(Subtotal*.01),0)
    MemDisc=?(ShipCode="F", 0, MemDisc) ;; fundraisers don't get member discounts
    AdjTotal=Subtotal-VolDisc-MemDisc
    ...
    «$Shipping» = ?(ShipCode = "F", 0, «$Shipping») ;; fundraisers don't pay shipping
...
case OrderNo > 400000 and OrderNo < 500000
    ...
    if Taxable="Y"
        // enabled sales tax recalculation for refiguring orders.
        call ".salestax"
    endif
    
    if «$Shipping»>25.00  //and MemDisc>0
        rayj="N"
...
arrayfilter newarray, newarray, ¶, arrayinsert(extract(newarray,¶,seq()),4,2,¬)
